...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Bitcoin server has now stopped running!";
}
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Bitcoin server is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Bitcoin server is downloading blocks...");
